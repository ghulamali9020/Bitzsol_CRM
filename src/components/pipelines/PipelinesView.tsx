  ...
  return (
    <div className="space-y-5">
      <div className="animate-fade-in-up flex items-center justify-between">
        <div>
          <h3 className="text-xl font-black text-brand-solid">Pipelines</h3>
      ...
      {pipelines.length === 0 ? (
        <div className="animate-fade-in-up glass rounded-2xl text-center py-16 shadow-md border border-crm-border/30">
          ...
          {pipelines.map((p, i) => {
            return (
              <div
                ...
                className="stagger-item animate-fade-in-up card-hover glass p-5 rounded-2xl border border-crm-border/30 hover:border-[#0164DA]/30 hover:shadow-lg transition-all duration-300 group shadow-md"
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="w-10 h-10 rounded-xl bg-[#0164DA]/10 text-[#0164DA] flex items-center justify-center transition-transform duration-300 group-hover:scale-110">
                    <Layers className="w-5 h-5" />
                  </div>
                  {user?.role === "admin" && (
                    <div className="flex gap-1.5">
                      <button onClick={() => openEdit(p)} className="w-7 h-7 rounded-lg bg-[#0164DA]/10 text-[#0164DA] flex items-center justify-center hover:bg-[#0164DA]/20 cursor-pointer transition-colors">
                        <Pencil className="w-3.5 h-3.5" />
                      </button>
                      <button onClick={() => setDeleteId(p.id)} className="w-7 h-7 rounded-lg bg-red-500/10 text-red-400 flex items-center justify-center hover:bg-red-500/20 cursor-pointer transition-colors">
      ...
      {showForm && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-md p-4 animate-in fade-in duration-200">
          <div className="bg-crm-panel rounded-2xl border border-crm-border max-w-md w-full shadow-2xl text-crm-text-main animate-in fade-in duration-200 overflow-hidden">
            <div className="px-6 py-4 bg-linear-to-r from-[#0164DA]/10 via-crm-panel to-[#0164DA]/5 border-b border-crm-border">
              <h4 className="text-base font-black text-brand-solid">{editPipeline ? "Edit Pipeline" : "New Pipeline"}</h4>
            </div>
            <div className="p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input type="text" required value={name} onChange={(e) => setName(e.target.value)}
                  className="w-full px-4 py-2.5 rounded-xl bg-crm-input-bg border border-crm-border text-crm-text-main focus:outline-none focus:border-[#0164DA] focus:ring-2 focus:ring-[#0164DA]/20 text-sm transition-all"
              ...
              <div>
                ...
                <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3}
                  className="w-full px-4 py-2.5 rounded-xl bg-crm-input-bg border border-crm-border text-crm-text-main focus:outline-none focus:border-[#0164DA] focus:ring-2 focus:ring-[#0164DA]/20 text-sm resize-none transition-all"
                  placeholder="Optional description..." />
              </div>
              <div className="flex gap-3">
                <button type="button" onClick={() => setShowForm(false)} className="flex-1 py-2.5 rounded-xl bg-crm-panel-hover hover:bg-crm-panel border border-crm-border text-sm font-semibold cursor-pointer transition-colors">Cancel</button>
                <button type="submit" disabled={loading} className="flex-1 py-2.5 rounded-xl btn-brand-gradient hover:opacity-95 active:scale-95 text-white text-sm font-bold disabled:opacity-50 cursor-pointer shadow-lg shadow-[#0164DA]/20 transition-all">
